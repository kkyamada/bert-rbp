...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            #if self.output_attentions and i<10:
            #    all_attentions = all_attentions + (layer_outputs[1],)
...
@add_start_docstrings(
    "The bare Bert Model transformer outputting raw hidden-states without any specific head on top. This model takes two different inputs by applying 1dconv to one of the inputs and concatenating them together",
    BERT_START_DOCSTRING,
)
class BertRNARBPModel(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.encoder = BertEncoder(config)
        self.pooler = BertPooler(config)
        self.conv1d = nn.Conv1d(in_channels=config.conv1d_in_channels,
                                out_channels=config.conv1d_out_channels,
                                kernel_size=config.conv1d_kernel_size,
                                stride=config.conv1d_stride,
                                padding=config.conv1d_padding,
                                dilation=config.conv1d_dilation,)
    ...
    def forward(
        ...
        r"""
        inputs_rna (:obj:`torch.cuda.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`, defaults to :obj:`None`):
            You can choose to directly pass an embedded representation.This is useful if you want more control over how to convert `input_ids` indices into 
            associated vectors than the model's internal embedding lookup matrix. This is sub-type of inputs_embeds in BertModel and requires inputs_rbp.
        inputs_rbp (:obj:`torch.cuda.FloatTensor` of shape :obj:`(sequence_length, hidden_size)`, `optional`, defaults to :obj:`None`):
            You can choose to directly pass an embedded representation. This is useful if you want more control over how to convert `input_ids` indices 
            into associated vectors than the model's internal embedding lookup matrix. This is used in combination with inputs_rna.
    ...
    Examples::
        ...
        #oirignal inputs_rbp.size() = "length of RBP", "hidden dimension of proteinBERT"
        inputs_rbp = torch.unsqueeze(inputs_rbp, dim=1)
        inputs_rbp = self.conv1d(inputs_rbp)
        #inputs_rbp = F.relu(self.conv1d(inputs_rbp))
        inputs_rbp = inputs_rbp.view(inputs_rbp.size()[1], inputs_rbp.size()[0], inputs_rbp.size()[2])
        inputs_rbp = inputs_rbp.repeat(inputs_rna.size()[0], 1, 1) #inputs_rbp.size() = "batch_size", "length of RBP", "hidden dimension=768"
        
        
        inputs_embeds = torch.cat((inputs_rna, inputs_rbp), dim=1)
...
class BertForRNARBPClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels

        self.bert = BertRNARBPModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    Examples::
        ...
        outputs = self.bert(
            ...
            position_ids=position_ids,
            head_mask=head_mask,
            inputs_rna=inputs_rna,
            inputs_rbp=inputs_rbp,
        )
